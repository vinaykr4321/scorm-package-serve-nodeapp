...
/*	 window.parent.postMessage({
                ...
                arguments: [strExitType, blnStatusWasSet]
        }, '*');
	return true;*/
...
//	debugger;
	if(event.data['element']){
		localStorage.setItem(event.data['element'], event.data['value']);
	//	console.log(`Saving ${event.data['element']} to the LocalStorage with the value of ${event.data['value']}`);
	}
})
...
function custom_GetValue(element) {
	var result = localStorage.getItem(element);
	if (result) {
		return result;
	} else {
		custom_GetValue(element);
	}
}


var setIntervalSynchronous = function (func, element, delay) {
...
function getValue(element,clear) {
    var result = localStorage.getItem(element);
//debugger;
    if (result) {
       window.getValueResult = result;
       clear(); 
    }
}
...
//	debugger;
	...
	var urlResult = url.searchParams.get(strElement);
	if (urlResult) {
		//return 'false';
		urlResult = urlResult.replaceAll('|sp|', ' ');
	}else{
	//vincheck
//		localStorage.removeItem(strElement);
		window.getValueResult = null;
console.log(`After Item remove: ${localStorage.getItem(strElement)}`);
		window.parent.postMessage({
                	function: "LMSGetValue",
	                arguments: [strElement]
	        }, '*');
		var allGetValueData = localStorage.getItem('allGetValueData');
		allGetValueData = JSON.parse(allGetValueData);
		urlResult = allGetValueData[strElement];
		console.log(`urlResult is: ${urlResult}`);
	}
	return urlResult;
